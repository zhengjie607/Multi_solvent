...
from spoolmanager import spoolmanager
...
class pronsole(cmd.Cmd):
    ...
    def promptf(self):
        ...
        if "%" not in promptstr:
            ...
            if self.p.printing:
                self.process=int(1000*float(self.p.queueindex)/len(self.p.mainqueue))/10
                #print("加工进度：",self.process)
...
(this will terminate the print)."))
    ...
    def _do_load(self, filename):
        ...
        if not os.path.exists(filename):
            ...
            return
        gcode=self._load_gcode(filename)
        self.load_gcode(filename,gcode=gcode)
    ...
    def parse_gcode(self,gcode):
        self.all_layers_for_opengl=[]#opengl绘制时用到的数据
        self.layer_height=[]#加工时显示的当前层高
        for i in range(2,len(gcode.all_layers)-2):
            lines=[]
            tmp_line=None
            line_len=len(gcode.all_layers[i])
            z=gcode.all_layers[i][0].z
            for j in range(line_len):
                if gcode.all_layers[i][j].command=='G0' and gcode.all_layers[i][j].x and gcode.all_layers[i][j].y:
                    if tmp_line:
                        lines.append(tmp_line)
                    tmp_line=[]
                    tmp_line.append((gcode.all_layers[i][j].x,gcode.all_layers[i][j].y,z))
                if gcode.all_layers[i][j].command=='G1' and  gcode.all_layers[i][j].x and gcode.all_layers[i][j].y:
                    tmp_line.append((gcode.all_layers[i][j].x,gcode.all_layers[i][j].y,z))
                if j==line_len-1:
                    lines.append(tmp_line)
            self.all_layers_for_opengl.append(lines)
            self.layer_height.append(z)
            ...
            if not self.lineignorepattern.match(l) and l[:4] != "wait" and not self.sdlisting \
               and not self.monitoring and (report_type == REPORT_NONE or report_type & REPORT_MANUAL):
                ...
                if self.p.printing:
                    ...
                    print("加工进度：",self.process)
                #sys.stdout.write(self.promptf())
                #print("self.promptf():",self.promptf())
                #sys.stdout.flush()

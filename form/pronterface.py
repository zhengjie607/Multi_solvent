...
Printrun. If not, see <http://www.gnu.org/licenses/>."""
    ...
    def connect(self, event = None):
        ...
        port = None
        if self.serialport.GetValue():#界面端口号输入值
            port = str(self.serialport.GetValue())
        else:
            scanned = self.scanserial()#扫描所有可用端口号，返回名称的列表
            if scanned:
                port = scanned[0]
        baud = 115200
        try:
            baud = int(self.baud.GetValue())#界面波特率输入值
        except:
            self.logError(_("Could not parse baud rate: ")
                          + "\n" + traceback.format_exc())
        if self.paused:#在这个类中，init的self.pasued=False，不知何时为True？？？？？
            self.p.paused = 0#self.p是printcore.printcore()
            self.p.printing = 0
            ...
            self.paused = 0
            if self.sdprinting:
                self.p.send_now("M26 S0")#send_now是将p中的priqueue调用函数put_nowait
        #调用pronsole中的connect_to_printer，大概意思就是链接serial，并开始线程
